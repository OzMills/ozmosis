...
const screenshotRoot = path.join(root, "docs", "qa", "screenshots", "v0.85.3.6-feedback-rhythm");
const resultPath = path.join(root, "docs", "qa", "v0.85.3.6_feedback_rhythm_results.json");
const comparisonPath = path.join(screenshotRoot, "comparison.html");
const profileDir = fs.mkdtempSync(path.join(os.tmpdir(), "ozmosis-v08536-feedback-rhythm-edge-profile-"));
const keepProfile = process.argv.includes("--keep-profile");

const expectedStartAppVersion = "v0.85.3.5 - Shared Practice Frame Convergence Pass";
const expectedStartExportVersion = "v0.85.3.5-shared-practice-frame-convergence";
const expectedFinalAppVersion = "v0.85.3.6 - Feedback Rhythm Pass";
const expectedFinalExportVersion = "v0.85.3.6-feedback-rhythm";
...
async function b1Answer(cdp, mode) {
  return evaluate(cdp, `(() => {
    ...
    input.value = ${JSON.stringify(mode)} === "correct" ? answer : "zzzz falsch";
...
function comparisonHtml() {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><title>Ozmosis v0.85.3.6 feedback rhythm contact sheet</title><style>
body{margin:0;background:#050914;color:#dff8ff;font-family:Arial,sans-serif;padding:18px}
h1{font-size:20px;margin:0 0 14px}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:16px;align-items:start}
...
</style></head><body><h1>Ozmosis v0.85.3.6 Feedback Rhythm</h1><div class="grid">${entries}</div></body></html>`;
}
...
async function runBrowserChecks() {
  ...
  try {
    ...
    await screenshot(cdp, "choice-fail-before-after-style");
    ...
    result.exportImport = await evaluate(cdp, `(() => {
      ...
      const oldPayload = Object.assign({}, payload, { version:"v0.85.3.5-shared-practice-frame-convergence", appVersion:"v0.85.3.5 - Shared Practice Frame Convergence Pass" });
    ...
    for (const [width, height] of viewports) {
      ...
      const check = await evaluate(cdp, `(() => ({
        ...
        confidenceVisible:!!(document.getElementById("b1ConfidenceRow") && !document.getElementById("b1ConfidenceRow").hidden),
...
async function main() {
  try {
    ...
    assert(result.runtimeVersionUpdated, "Runtime version has not been updated to v0.85.3.6.");
    ...
    result.finalDecision = "FEEDBACK_RHYTHM_ACCEPTED";
    ...
    if (/article grid/i.test(String(error && error.message || error))) {
      ...
      result.nextPatch = "v0.85.3.6a - Article Grid Feedback Regression Repair";
    } else if (/Edge|CDP|browser/i.test(String(error && error.message || error))) {
      result.finalDecision = "BLOCKED_BROWSER_CAPTURE_FAILED";
      result.nextPatch = "v0.85.3.6a - Feedback Rhythm Repair";
    } else {
      result.finalDecision = "FEEDBACK_RHYTHM_PARTIAL_NEEDS_REPAIR";
      result.nextPatch = "v0.85.3.6a - Feedback Rhythm Repair";
    }
