...
- **Not enough evidence**  
  Some evidence exists, but it is too thin, scaffolded, diagnostic, self-marked, or otherwise limited for a useful claim.
...
Future durability, review, repair, and pattern labels must remain inactive until a later patch defines and validates them. v0.60 uses only cautious "Possible pattern" language for repeated error signals.
...
Completed: add simple rules-based recommendation logic and compact Learning Map guidance while preserving conservative claim limits.
...
```txt
Suggested Next Practice now has explicit QA guardrails for thin evidence, easy-task inflation, scaffolded-majority evidence, diagnostic-only evidence, self-marked production-only evidence, broad Wortschatz sparsity, and repair/contrast overreach. Debug output exposes additive evidenceQuality and inflationRisk metadata. Learner-facing recommendation copy remains cautious, uses existing routes only, and states closest available route where an exact task route is not available. Scoring, storage keys, export/import compatibility, content banks, B1-only scope, route inventory, and Today's B1 Sprint composition remain unchanged.
```

### v0.57.1  -  Big UX Update

Perform a full mobile-first UX and visual-system pass across the current app without changing learning semantics.

Completed in v0.57.1:
...
```txt
Ozmosis now adds additive Skill Matrix QA metadata for sample size, evidence quality, support mix, inflation risks, and cell limitations. Matrix cells remain conservative when evidence is thin, scaffolded, diagnostic-only, self-marked production-only, derived-only, or unknown/legacy task depth evidence. Mobile matrix cards and desktop table output show compact limitation labels while preserving scoring, evidence semantics, storage keys, export/import compatibility, content banks, B1-only scope, route composition, and Today's B1 Sprint composition.
```
...
```txt
Ozmosis stabilised the Learning Map, Skill Matrix, Suggested Next Practice, debug snapshot, tester report summary, current docs, and export metadata around the conservative evidence model. v0.57.2 flow/navigation/mobile UX corrections and v0.58 matrix QA metadata remain intact. Scoring, evidence semantics, storage keys, export/import compatibility, content banks, B1-only scope, route composition, and Today's B1 Sprint composition remain unchanged.
```
