# OZMOSIS — UPDATED SOURCE BRIEF AND DEVELOPMENT TIMELINE
...
`docs/OZMOSIS_DEVELOPMENT_ROADMAP.md`
...
Check whether existing B1 banks consistently expose:
- what skill is tested
- what task type is used
- what grammar/vocabulary/function focus is represented
- which B1 skill cluster each item contributes to

Keep this compatible with the existing static app and localStorage approach.
Update source comments/docs where needed.
Preserve all current functionality unless a change is explicitly required.
```
